...
The portfolio includes an AI chat assistant powered by Google's Gemini API. The assistant is designed to answer questions about Amelito, his skills, projects, and other information related to the portfolio.

## Prerequisites

1. A Google account
2. A Gemini API key (free tier available)
...
2. The chat interface will automatically use this API key to communicate with the Gemini API.
...
The default system prompt is defined in `src/services/GeminiService.ts` and includes instructions for the AI about:
...
To customize how the AI assistant responds, you can modify the system prompt in the `GeminiService.ts` file:
...
After setting up the Gemini API key and customizing the system prompt, you can test the AI assistant by:
...
1. Make sure the Gemini API key is correctly set in the `.env` file
2. Check the browser console for any error messages
3. Verify that the system prompt is properly formatted
4. Ensure that the Gemini API is available and not experiencing any outages
...
- Clickable links for websites, email addresses, and phone numbers (styled with blue color and underline)
- Formatted text (bold, italic, etc.)
- Lists and other structured content

The styling for links is already configured in the ChatInterface.vue component, making them stand out with a blue color and underline for better visibility.
...
- Links will automatically appear in blue with an underline for better visibility
- Use <strong> for emphasis and <em> for subtle emphasis
- Use <ul> and <li> for lists
`
```

The CSS styling for these links is defined in the ChatInterface.vue component:
